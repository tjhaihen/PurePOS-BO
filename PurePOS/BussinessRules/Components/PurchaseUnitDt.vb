...
#Region "Others Function"
        ...
        Public Function ValidateGetItemPU() As DataTable
            ...
            cmdToExecute.CommandText = "SELECT dt.ID, " & _
                                        ...
                                        "dt.IsBonus " & _
                                        "FROM PurchaseUnitHd hd " & _
                                        "inner join PurchaseUnitdt dt ON dt.PUnitNO = hd.PUnitNO And (dt.Qty * dt.itemfactor) > dt.QtyReturn " & _
                                        "LEFT JOIN " & _
                                        "( " & _
                                         "SELECT prd.IDPU, sum(prd.Qty * prd.ItemFactor) AS QtyPURT " & _
                                         "FROM PurchaseReturnHd prh " & _
        ...
        Public Function SelectLastOneByItemAndItemUnit(ByVal Currency As String) As DataTable
            Dim cmdToExecute As SqlCommand = New SqlCommand
            cmdToExecute.CommandText = "SELECT TOP 1 pud.* " & _
                                       "FROM PurchaseUnitHd puh " & _
                                       "INNER JOIN PurchaseUnitDt pud ON pud.PUnitNO = puh.PUnitNO " & _
                                       "WHERE puh.IsDeleted = 0 And puh.IsApproval = 1 " & _
                                       "AND pud.IsDeleted = 0 AND IsBonus = 0 and puh.Currency = @Currency and pud.ItemSeqNo = @ItemSeqNo AND pud.ItemUnitID = @ItemUnitID " & _
                                       "ORDER BY puh.PUnitNO DESC "
            cmdToExecute.CommandType = CommandType.Text
            Dim toReturn As DataTable = New DataTable("PurchaseUnitdt")
            Dim adapter As SqlDataAdapter = New SqlDataAdapter(cmdToExecute)
        ...
        Public Function SelectOneMaxPriceByItemAndItemUnit() As DataTable
            Dim cmdToExecute As SqlCommand = New SqlCommand
            cmdToExecute.CommandText = "SELECT TOP 1 " & _
                                       "pud.ID, " & _
                                       "pud.PUnitNO, " & _
                                       "pud.POrdNO, " & _
                                       "pud.ItemSeqNo, " & _
                                       ...
                                       "pud.ItemFactor, " & _
                                       "pud.Qty, " & _
                                       "(pud.Price * puh.CurrencyRate) AS Price, " & _
                                       "pud.Disc1Pct, " & _
                                       "(pud.Disc1Amt * puh.CurrencyRate) AS Disc1Amt, " & _
                                       "pud.Disc2Pct, " & _
                                       "(pud.Disc2Amt * puh.CurrencyRate) AS Disc2Amt, " & _
                                       "pud.Disc3Pct, " & _
                                       "(pud.Disc3Amt * puh.CurrencyRate) AS Disc3Amt, " & _
                                       "pud.QtyReturn, " & _
                                       "pud.UserInsert, " & _
                                       "pud.InsertDate, " & _
                                       "pud.UserUpdate, " & _
                                       ...
                                       "INNER JOIN PurchaseUnitDt pud ON pud.PUnitNO = puh.PUnitNO " & _
                                       "WHERE puh.IsDeleted = 0 And puh.IsApproval = 1 " & _
                                       "AND pud.IsDeleted = 0 AND IsBonus = 0 " & _
                                       "and pud.ItemSeqNo = @ItemSeqNo AND pud.ItemUnitID = @ItemUnitID " & _
                                       "ORDER BY (pud.Price * puh.CurrencyRate) DESC "
            cmdToExecute.CommandType = CommandType.Text
            Dim toReturn As DataTable = New DataTable("PurchaseUnitdt")
            Dim adapter As SqlDataAdapter = New SqlDataAdapter(cmdToExecute)
        ...
        Public Function SelectLastNPurchase(ByVal n As String) As DataTable
            Dim cmdToExecute As SqlCommand = New SqlCommand

            cmdToExecute.CommandText = "SELECT TOP " & n.Trim & " hd.PUnitNo, hd.PUnitDate, hd.EntitiesSeqNo, " & _
                                        "(SELECT EntitiesName FROM Entities WHERE EntitiesSeqNo = hd.EntitiesSeqNo) AS EntitiesName, " & _
                                        "dt.ItemUnitID, (SELECT ItemUnitName FROM ItemUnit WHERE ItemUnitID = dt.ItemUnitID) AS ItemUnitName, " & _
                                        "dt.ItemFactor, dt.Qty, dt.Price, " & _
                                        "NettUnitPrice = " & _
                                        "CASE WHEN(hd.IsPPN = 0) " & _
                                        "THEN((dt.Price - dt.Disc1Amt - dt.Disc2Amt - dt.Disc3Amt)) " & _
                                        "ELSE(((dt.Price - dt.Disc1Amt - dt.Disc2Amt - dt.Disc3Amt)) + " & _
                                        "((hd.PPNPct / 100) * ((dt.Price - dt.Disc1Amt - dt.Disc2Amt - dt.Disc3Amt))))" & _
                                        "END " & _
                                        "FROM PurchaseUnitDt dt " & _
                                        "INNER JOIN PurchaseUnitHd hd ON hd.PUnitNo = dt.PUnitNo " & _
                                        "WHERE dt.IsDeleted = 0 AND dt.ItemSeqNo = @ItemSeqNo ORDER BY hd.InsertDate DESC"
            cmdToExecute.CommandType = CommandType.Text
        ...
        Public Function SelectLastOnePurchase() As Decimal
            Dim decLastPurchasePrice As Decimal = 0D
            Dim cmdToExecute As SqlCommand = New SqlCommand

            cmdToExecute.CommandText = "SELECT TOP 1 hd.PUnitNo, hd.PUnitDate, hd.EntitiesSeqNo, " & _
                                        "(SELECT EntitiesName FROM Entities WHERE EntitiesSeqNo = hd.EntitiesSeqNo) AS EntitiesName, " & _
                                        "dt.ItemUnitID, (SELECT ItemUnitName FROM ItemUnit WHERE ItemUnitID = dt.ItemUnitID) AS ItemUnitName, " & _
                                        "dt.ItemFactor, dt.Qty, dt.Price, " & _
                                        "NettUnitPrice = " & _
            ...
            Try
                ...
                If toReturn.Rows.Count > 0 Then
                    decLastPurchasePrice = CType(ProcessNull.GetDecimal(toReturn.Rows(0)("NettUnitPrice")), Decimal)
                End If

                Return decLastPurchasePrice
            Catch ex As Exception
                ExceptionManager.Publish(ex)
            Finally
                _mainConnection.Close()
