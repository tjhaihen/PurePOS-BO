...
#Region " Custom Functions "
        Public Function SelectByItemCategoryID(Optional ByVal IsActiveOnly As Boolean = False) As DataTable
            Dim cmdToExecute As SqlCommand = New SqlCommand
            If IsActiveOnly = False Then
                cmdToExecute.CommandText = "SELECT * FROM ItemSubCategory WHERE ItemCategoryID = @ItemCategoryID"
            Else
                cmdToExecute.CommandText = "SELECT * FROM ItemSubCategory WHERE ItemCategoryID = @ItemCategoryID AND isActive = 1"
            End If
            cmdToExecute.CommandType = CommandType.Text
