...
#Region " Custom Functions "
        ...
        Public Function SelectOneRangeByMarginRangeID() As DataTable
            Dim cmdToExecute As SqlCommand = New SqlCommand

            cmdToExecute.CommandText = "SELECT TOP 1 * " & _
                                       "FROM MarginRangeHd mrh " & _
                                       "INNER JOIN MarginRangeDt mrd ON mrd.MarginRangeID = mrh.MarginRangeID " & _
                                       "WHERE mrh.IsActive = 1 And mrd.IsActive = 1 " & _
                                       "AND mrh.MarginRangeID = @MarginRangeID " & _
                                       "AND mrd.MaxAmount <= @MaxAmount " & _
                                       "ORDER BY mrd.MaxAmount DESC "
            cmdToExecute.CommandType = CommandType.Text
        ...
        Public Function SelectByMarginRangeIDAndMaxAmount() As DataTable
            Dim cmdToExecute As SqlCommand = New SqlCommand

            cmdToExecute.CommandText = "SELECT * FROM MarginRangedt WHERE MarginRangeID = @MarginRangeID " & _
                                        "AND MaxAmount = @MaxAmount " & _
                                        "ORDER BY MaxAmount DESC, MarginPct DESC"
            cmdToExecute.CommandType = CommandType.Text
